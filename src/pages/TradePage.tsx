  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      const newBalance = tradeType === "buy"
        ? wallet.balance - Number(amountGBP)
        : wallet.balance + Number(amountGBP)
      ...
      setSuccess(
        `${tradeType === "buy" ? "Bought" : "Sold"} ${coinAmount.toFixed(6)} ${selectedCoin} for £${amountGBP}`
      )
      setAmountGBP("")
      fetchHoldings()
      ...
      <section className="content">
        <div style={{ maxWidth: "560px", margin: "0 auto", padding: "24px" }}>
          ...
          <div style={s.card}>
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <div style={{ display: "flex", gap: "8px", marginTop: "6px" }}>
                {COINS.map(coin => (
                  <button
                    key={coin}
                    onClick={() => { setSelectedCoin(coin); setAmountGBP(""); setError(null); setSuccess(null) }}
            ...
            {calculateCoin() !== null && (
              <div style={s.preview}>
                <span>You will {tradeType === "buy" ? "receive" : "sell"}</span>
                <strong>{calculateCoin()!.toFixed(6)} {selectedCoin}</strong>
              </div>
            )}
            ...
            >
              {loading
                ? "Processing..."
                : `${tradeType === "buy" ? "Buy" : "Sell"} ${selectedCoin} for £${amountGBP || "0"}`
              }
            </button>
